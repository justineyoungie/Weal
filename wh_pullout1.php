...
if (isset($_POST['pullout'])){
	...
	$query = "SELECT * FROM rsdetails WHERE rsNumber = '".$rsID."' ";
	$result = mysqli_query($dbc, $query);
	$error = false;
	while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		$queryA = "SELECT * FROM inventory WHERE materialID = '".$row['materialID']."' ";
		$resultA = mysqli_query($dbc, $queryA);
		$rowA = mysqli_fetch_array($resultA, MYSQLI_ASSOC);
		$qty = $rowA['quantity'] - $row['quantity'];
		
		if ($qty < 0){
			$error = true;
		}else{
			$queryB = "UPDATE inventory SET quantity = '".$qty."' WHERE materialID = '".$row['materialID']."' ";
	...
	if (!$error){
		$query = "UPDATE requisitionslip SET rsStatusID = '7' WHERE rsNumber = '".$rsID."' ";
		$result = mysqli_query($dbc, $query);
		if (!$result){
			$error = true;
		}
	...
	if (!$error){
		$message = '<div class="alert alert-success alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
					...
					You have successfully pulled out a Requisition Slip.
				</div>';
		...
		$message = '<div class="alert alert-danger alert-dismissable">
					...
					<h4><i class="icon fa fa-ban"></i>FAILED!</h4>
						Pulling out requisition slip unsuccessful!
					</div>';
	}
}
                ...
                <div class="row">
					...
					<br>
                        <div class="col-xs-12">
							...
							echo '
							<form action = "wh_pullout1.php?r='.$rsID.'" method = "post">';
							
							if ($notYet){
								echo '<input type="submit" style = "width:170px; margin:5px;" class="btn btn-success pull-right" value = "Pull-Out Request" name = "pullout">';
							}
							?>
							</form>
							<a href = "wh_pullout.php"><button type="button" style = "width:100px; margin:5px;" class="btn btn-warning pull-right">Back</button></a>
